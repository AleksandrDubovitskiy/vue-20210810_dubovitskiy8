<template>
  <form class="meetup-form" @submit.prevent="$emit('submit', sumbit())">
    <div class="meetup-form__content">
      <fieldset class="meetup-form__section">
        ...
        <ui-form-group label="Дата">
          <ui-input-date v-model="localMeetup.date" type="date" name="date" :value="localMeetup.date" />
        </ui-form-group>
        <ui-form-group label="Место">
          <ui-input v-model="localMeetup.place" name="place" :value="localMeetup.place" />
        </ui-form-group>
      ...
      <meetup-agenda-item-form
        ...
        @update:agenda-item="updatedAgendaItem"
      />
...
export default {
  ...
  methods: {
    ...
    removeAgendaItem(agendaId) {
      let updateAgendaList = this.localMeetup.agenda.filter((el) => el.id !== agendaId);
      this.localMeetup.agenda = updateAgendaList;
    },
    updatedAgendaItem(newAgendaItem) {
      const index = this.localMeetup.agenda.findIndex((agendaItem) => agendaItem.id === newAgendaItem.id);
      this.localMeetup.agenda.splice(index, 1, newAgendaItem);
    },
    sumbit() {
      return _.cloneDeep(this.localMeetup);
    },
  },
};
</script>
